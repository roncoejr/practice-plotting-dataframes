...
tnames_df <- con |> read_adbc("SELECT \"name\" FROM TABLE(RESULT_SCAN())") |>
  tibble::as_tibble()

consumptionsample_df <- con |> read_adbc("SELECT * FROM CONSUMPTION_SAMPLE_V1") |>
  tibble::as_tibble()
...
dataset <- data.frame(
                      independent = c('2023-01-20', '2023-02-22', '2023-03-10', '2023-04-10'),
...
dataset %>% ggplot(aes(independent, the_data, colour = treatment))+
  ...
  labs(title = "A Test Graph of a Simple DataFrame",
       x = "Date Range",
       y = "Deal Value")+
  theme_bw()
...
consumptionsample_df %>% ggplot(aes(USAGEDATE, CREDITSCONSUMED, colour = CREDITSCONSUMED))+
  geom_point(size = 3, alpha = 0.8)+
  geom_smooth(method = lm)+
  labs(title = "Sample Consumption Visualizer",
       x = "Year over Year",
       y = "Credits Consumed")
...
cons_df <- consumptionsample_df %>%
  mutate(use_month = month(USAGEDATE)) %>%
  mutate(use_year = year(USAGEDATE)) %>%
  mutate(cost_per_query_value = gsub("\\$", "", COSTPERQUERY)) %>%
  mutate(cost_per_query_value = as.double(cost_per_query_value)) %>%
  select(USAGEDATE, use_month, use_year, cost_per_query_value)

names(cons_df)

cons_df

cons_df %>% ggplot(aes(x = USAGEDATE, y = cost_per_query_value, color = use_month))+
  geom_point()+
  geom_smooth()+
  labs(title = "Consumption by Months",
       x = "Months",
